...
int get_next_int()
{
    ...
    while (true)
    {
        ...
        if (scanf("%s", buf) != 1)
        {
            printf("Input reading error in get_next_int!\n");
            exit(1);
        }
        // skip token£® "p"°¢"edge"°¢"e"£©
    }
...
void build_instance(char *file)
{
	...
	char first[32];
	if (scanf("%s", first) == 1 && strcmp(first, "p") == 0)
	{
		char edge_str[32];
		scanf("%s", edge_str);         
		n = get_next_int();            
...
{
	tau[c] = tau[c] + (delta_tau * 2) / 3;//Êõ¥Êñ∞ËØ•ÈõÜÂêàÁöÑ‰ø°ÊÅØÁ¥†tau
}
